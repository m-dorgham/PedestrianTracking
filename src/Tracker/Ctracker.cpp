        ...
        )
{
    ...
    if (m_prevFrame.size() == frame.size())
    {
        if (m_useLocalTracking)
        {
            m_localTracker.Update(tracks, m_prevFrame, frame);
        }
    }
    ...
    if (tracks.size() == 0)
    {
        ...
        {
            tracks.push_back(make_unique<CTrack>(detections[i],
                                                      regions[i],
                                                      m_kalmanType,
                                                      dt,
                                                      accelNoiseMag,
    ...
    for (size_t i = 0; i < assignment.size(); i++)
    {
        ...
        if (assignment[i] != -1) // If we have assigned detect, then update using its coordinates,
        {
            ...
            tracks[i]->Update(detections[assignment[i]], regions[assignment[i]], true, max_trace_length, m_prevFrame, frame);
        }
        else				     // if not continue using predictions
        {
            tracks[i]->Update(Point_t(), CRegion(), false, max_trace_length, m_prevFrame, frame);
        }
    }
